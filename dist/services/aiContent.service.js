...
exports.getAiContentByLead = getAiContentByLead;
exports.listAiContent = listAiContent;
const openai_1 = require("../config/openai");
const AiContent_model_1 = require("../models/AiContent.model");
// ---------------------------------------------------------------------------
...
// Context builder
// ---------------------------------------------------------------------------
function buildContextLines(ctx, scrape, subject, instructions) {
    const lines = [];
    if (ctx.businessName)
        lines.push(`Business Name: ${ctx.businessName}`);
    if (ctx.contactPerson)
        lines.push(`Contact Person: ${ctx.contactPerson}`);
    if (ctx.position)
        lines.push(`Their Position: ${ctx.position}`);
    if (ctx.categoryName)
        lines.push(`Industry / Category: ${ctx.categoryName}`);
    if (ctx.companySize)
        lines.push(`Company Size: ${ctx.companySize}`);
    ...
    if (ctx.techStack)
        lines.push(`Tech Stack They Use: ${ctx.techStack}`);
    if (ctx.businessInfo)
        lines.push(`Business Info: ${ctx.businessInfo}`);
    if (ctx.painPoints)
        lines.push(`Pain Points / Opportunities: ${ctx.painPoints}`);
    if (ctx.description)
        lines.push(`Additional Context: ${ctx.description}`);
    if (ctx.offerSummary)
        lines.push(`What We Are Offering: ${ctx.offerSummary}`);
    if (ctx.senderName)
        lines.push(`Sender Name: ${ctx.senderName}`);
    if (ctx.senderCompany)
        lines.push(`Sender Company: ${ctx.senderCompany}`);
    if (subject)
        lines.push(`Preferred Subject Hint: ${subject}`);
    if (instructions)
        lines.push(`Extra Instructions: ${instructions}`);
    if (scrape) {
        lines.push(`\n--- Website Content (${scrape.url}) ---`);
        if (scrape.title)
            lines.push(`Page Title: ${scrape.title}`);
...
// Company analyser
// ---------------------------------------------------------------------------
async function analyzeCompany(ctx, scrape, outreachType) {
...
async function generateMessage(ctx, scrape, type, outreachType, tone, language, analysis, subject, instructions) {
...
async function generateContent(input) {
    ...
    const { subject, body } = await generateMessage(ctx, websiteScrape, type, outreachType, tone, language, analysis, input.subject, input.instructions);
    // 4. Persist
    if (input.createdBy) {
        await AiContent_model_1.AiContent.create({
            leadId: input.leadId,
            type,
            tone,
            language,
...
async function getAiContentByLead(leadId) {
    return AiContent_model_1.AiContent.find({ leadId })
        .populate('createdBy', 'name email')
        .sort({ createdAt: -1 })
        .lean();
}
async function listAiContent(filters) {
    const query = {};
    if (filters.leadId)
    ...
    return AiContent_model_1.AiContent.find(query)
        .populate('createdBy', 'name email')
        .sort({ createdAt: -1 })
        .lean();
}
//# sourceMappingURL=aiContent.service.js.map