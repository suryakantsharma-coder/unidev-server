...
// ─── Helper ───────────────────────────────────────────────────────────────────
async function createTaskAgentSession(voice) {
    const inputTranscription = env_1.env.realtimeEnableInputTranscription
        ? { transcription: { model: env_1.env.REALTIME_INPUT_TRANSCRIPTION_MODEL } }
        : { transcription: null };
...
router.post('/session', async (req, res, next) => {
    try {
        ...
        const ephemeralKey = (body.value ?? body.client_secret?.value);
...
router.post('/calls', async (req, res, next) => {
    try {
        ...
        const ephemeralKey = (sessionBody.value ?? sessionBody.client_secret?.value);
        if (!ephemeralKey) {
...
//# sourceMappingURL=taskAgentVoice.routes.js.map